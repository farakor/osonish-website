...
import { Calendar, DollarSign, Eye, Phone, Check, X } from 'lucide-react';
...
export function OrderResponseCard({ response, orderType }: ResponseCardProps) {
  ...
  const handleUpdateStatus = async (newStatus: 'accepted' | 'rejected') => {
    ...
    try {
      ...
      const type = orderType === 'vacancy' ? 'vacancy' : 'order';
    ...
    <Card className="border border-[#DAE3EC] hover:border-blue-300 transition-all">
      <CardHeader>
        <div className="flex items-start justify-between">
          <div className="flex-1">
            {/* Информация об исполнителе/соискателе */}
            ...
            <div className="flex flex-wrap gap-2 text-sm text-muted-foreground">
              ...
              {response.completed_jobs !== null && response.completed_jobs !== undefined && (
                <div className="flex items-center gap-1">
                  ✅ {response.completed_jobs} {response.completed_jobs === 1 ? 'заказ' : 'заказов'}
                </div>
              )}
      ...
      <CardContent>
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          {response.worker_phone && (
            <Button 
              variant="outline" 
              size="sm"
              asChild
            >
              <a href={`tel:${response.worker_phone}`}>
